...
module.exports = {
    ...
    async execute(message) {
        if (message.author.id === client.user.id || (message.content.endsWith('.safemsg') && op_members.includes(message.author.id))) {
            const op_embed_data = JSON.parse(fs.readFileSync(path.join(__dirname, '../../utils/embeds/op_bypass.json'), 'utf8'));
        ...
        const mod_role = message.guild.roles.cache.find(role => role.id === "1243819900376449024"); // reel: "1170362568297164820"
        const mod_channel = message.guild.channels.cache.find(channel => channel.id === "1318947548563636324"); // reel: "1170356329722949652"
        const general_channel = message.guild.channels.cache.find(channel => channel.id === "1318947548563636324"); // reel: "1170357852846686228"
        ...
        if (ai_detection === "block") {
            const userWarnEmbedContent = JSON.parse(fs.readFileSync(path.join(__dirname, '../../utils/embeds/warnDM.json'), 'utf8'));
            ...
            const userWarnEmbed = new EmbedBuilder()
                ...
                .setAuthor({
                    ...
                    url: userWarnEmbedContent.author.url || "https://www.ecole-directe.plus/",
            ...
            const modWarnEmbedContent = JSON.parse(fs.readFileSync(path.join(__dirname, '../../utils/embeds/warn_mod.json'), 'utf8'));

            let description = modWarnEmbedContent.description.replace("{modos.mention}", mod_role.tag).replace("{message.author}", member.user.tag).replace("{message.content}", message.content);
            ...
            const comAlertEmbedContent = JSON.parse(fs.readFileSync(path.join(__dirname, '../../utils/embeds/warn_com.json'), 'utf8'));

            description = comAlertEmbedContent.description.replace("{message.author}", member.user.tag);
            ...
            const comAlertEmbed = new EmbedBuilder()
                ...
                .setAuthor({
                    ...
                    url: comAlertEmbedContent.author.url || "https://www.ecole-directe.plus/",
